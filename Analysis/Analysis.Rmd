...
```{r}
clean_data <- read.csv("data/clean_dataset.csv")
...
```{r}
=======
```{r inscatter, echo=FALSE}
>>>>>>> Stashed changes

# Graph for train data


knitr::include_graphics(here::here("data/initial_scatter_plot.png"), rel_path = FALSE)
...
```{r,out.width="50%"}
 knitr::include_graphics(here::here("data/cp.png"), rel_path = FALSE)
```
=======
```{r inresult, echo=FALSE}
...
>>>>>>> Stashed changes
# Plot K Values
knitr::include_graphics(here::here("data/K-value_plot.png"), rel_path = FALSE)
```
...
>>>>>>> Stashed changes
# Exact K value
minK <- read.csv("data/minK.csv")
knitr::kable(minK, caption = "Minimum Value of K")     
```
We assigned the best $K$ to `k_min` with the use of Table \@ref(fig:kval), and use the best $K$ value to recreate our model following the same steps as before.
...
>>>>>>> Stashed changes
# Summary data
revenue_summary <- read.csv("data/revenue_summary.csv")
knitr::kable(revenue_summary, caption = "Summary data of analysis") 
```
At last, we visualize our prediction TABLE \@ref(fig:summary) using ggplot functions to evaluate if our model is overfitting, underfitting or a good fit for our target which is shown in Figure \@ref(fig:bplot).
...
>>>>>>> Stashed changes
# Regression Plot
knitr::include_graphics(here::here("data/revenue_Budget_plot.png"), rel_path = FALSE)
```
